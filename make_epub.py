...
# tmp/META-INF 目录
if not Path("tmp/META-INF").is_dir():
...
# 根据每章节生成内容, txt => html
for txt in txtlist:
    content=open(txt,'r').read().split('\n')
    content=[ '<div><p>%s</p></div>' % line for line in content]
    tf=open('tmp/OPS/%s.xhtml' % txt,'w')
    tf.write('''<html><head>
<title>%(title)s</title>
